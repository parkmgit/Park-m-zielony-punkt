...
export async function login(email: string, password: string): Promise<User | { error: string }> {
  try {
    const user = await queryOne<User>('SELECT * FROM users WHERE email = ? AND active = 1', [email]);
...
export async function register(data: RegisterData): Promise<User | { error: string }> {
  try {
    ...
    const existingUser = await queryOne('SELECT * FROM users WHERE email = ?', [data.email]);
    if (existingUser) {
    ...
    const newUser = await queryOne<User>('SELECT * FROM users WHERE email = ?', [data.email.toLowerCase()]);
